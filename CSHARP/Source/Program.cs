...
namespace HWID_Changer
{
    class Program
    {
        ...
        {
            // Default Interface Name on Windows Machines
            string interfaceName = "Ethernet";

            // Loop through all network interfaces to find the matching the adapterId supplied
            foreach (NetworkInterface i in NetworkInterface.GetAllNetworkInterfaces())
            {
                // If the adapterId matches, assign the corresponding interface name and exit
                if (i.Id == adapterId)
                {
            ...
            // Determine the control action based on the 'enable' parameter
            string control;
            if (enable)
                control = "enable";
            else
            ...
            // Prepare the command to execute using netsh to enable/disable the specified interface
